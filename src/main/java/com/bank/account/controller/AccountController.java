...
public class AccountController {
    ...
    public AccountController(AccountRepository accountRepository, BalanceRepository balanceRepository, CurrencyRepository currencyRepository, ExchangeRatesRepository exchangeRatesRepository) {
    ...
    public String exchangeCurrency(@PathVariable String iban, @PathVariable String from, @PathVariable String to, @PathVariable Double amount) {
        return internalCurrencyExchange(iban, from, to, amount);
    }
    ...
    private String internalAddToBalance(String iban, BalanceV1 balanceV1) {
    ...
    private String internalDebitFromBalance(String iban, BalanceV1 balanceV1) {
    ...
    private String internalCurrencyExchange(String iban, String from,  String to, Double amount) {
        Optional<Account> account = accountRepository.findByIban(iban);
        if (!account.isPresent()) {
            return "Account not found";
        }
